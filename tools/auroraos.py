...
def options(opts):
    opts.Add(BoolVariable("use_llvm", "Use the LLVM compiler - only effective when targeting AuroraOS", False))
...
def generate(env):
    ...
    env.Append(CPPDEFINES=["AURORAOS_ENABLED", "UNIX_ENABLED"])

    # Refer to https://github.com/godotengine/godot/blob/master/platform/auroraos/detect.py
    if env["lto"] == "auto":
        env["lto"] = "full"
